...
| **Host Test Entrypoint**       | **Missing**               | Needs to be created for `hl02`.                             |
...
Host-specific test files (e.g., `hosts/<host>/test.nix`) simply import this
generator and pass their logical `configuration.nix`.
...
### 3.3 Flake Integration & Dynamic Test Discovery (`config/nix/flake.nix`)
...
2.  Filters out directories that do not contain a `test.nix` file.
3.  For each valid host, imports its `test.nix` and formats it as a check
    attribute: `{ name = "<hostname>-test"; value = <test-derivation>; }`.
4.  Exposes these tests in the flake's `checks.${system}` output.
...
1.  **`detect-tests` (Discovery):**
    - Scans the `config/nix/hosts/` directory for any subdirectory containing a
      `test.nix` file.
    - Outputs a JSON array of host names (e.g., `["hl02"]`) to be used by the
