...
| [**Monitoring Alerting**](./monitoring-alerting.md)                         | Prometheus Alertmanager in the monitoring backend stack: Telegram notification routing, the severity model, the alert rules catalogue, and the highly available backend pair. | **Partially Implemented**: the single-host pipeline and rules catalogue are live; the HA pair (§3.3) is designed but not yet implemented. |
