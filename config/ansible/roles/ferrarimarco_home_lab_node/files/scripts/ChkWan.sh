...
Check_WAN() {
  ...
  if [ -n "$2" ]; then
    ...
    if { [ $RC18 -eq 18 ] || [ $RC -eq 0 ]; } && [ $(echo $RESULTS | cut -d',' -f5) -gt 0 ]; then # v1.14 RC=0 yet bytes transferred=0 WTF!!!! so treat as error.
      [ $RC18 -eq 18 ] && {
        echo -e $cBRED
        TXTINTERRUPTED="**Warning: INTERRUPTED; i.e. cut-short"
      } # v1.12 Mark interrupted transfer results as 'error'
      case "$2" in
      "$FORCE_WGET_10MB")
        Say "$TXTINTERRUPTED cURL $(($(echo $RESULTS | cut -d',' -f5) / 1000000))MByte transfer took:" $(printf "00:%05.2f secs @%6.0f B/sec" "$(echo $RESULTS | cut -d',' -f2)" "$(echo $RESULTS | cut -d',' -f3)")
        ;;
      "$FORCE_WGET_500B")
        Say "$TXTINTERRUPTED cURL $(($(echo $RESULTS | cut -d',' -f5)))Byte transfer took:" $(printf "00:%05.2f secs @%6.0f B/sec" "$(echo $RESULTS | cut -d',' -f2)" "$(echo $RESULTS | cut -d',' -f3)")
        ;;
      *)
...
if [ "$1" == "stop" ]; then
  if [ -f $LOCKFILE ]; then # v1.05
    ...
    [ -z "$(ps -w | grep $(basename $0) | grep -v "grep $(basename $0)" | grep -v "stop")" ] && {
      echo -e $cBCYA"\n\t"
...
flock -n $FD || {
  Say "$VER Check WAN monitor ALREADY running...ABORTing"
  exit
} # v1.15
