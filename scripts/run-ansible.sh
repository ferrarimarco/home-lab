...
elif [ "${ANSIBLE_VIEW_VAULT_FILE:-"false"}" = "true" ]; then
  ANSIBLE_VAULT_FILE_PATH="${ANSIBLE_DIRECTORY_INSIDE_CONTAINER_MOUNT_PATH}/${ANSIBLE_VAULT_FILE_PATH:-"inventory/group_vars/all/vault.yaml"}"
  ANSIBLE_COMMAND_VIEW_VAULT_FILE="ansible-vault view --vault-id ${ANSIBLE_VAULT_FULL_ID} ${ANSIBLE_VAULT_FILE_PATH}"
  COMMAND_TO_RUN="${COMMAND_TO_RUN} ${ANSIBLE_COMMAND_VIEW_VAULT_FILE}"
else
