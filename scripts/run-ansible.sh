...
if [ -n "${ANSIBLE_VAULT_PASSWORD:-""}" ]; then
  ...
  echo "Copying the contents of the ANSIBLE_VAULT_PASSWORD variable to ${ANSIBLE_VAULT_PASSWORD_FILE_PATH}"
  echo "${ANSIBLE_VAULT_PASSWORD}" >"${ANSIBLE_VAULT_PASSWORD_FILE_PATH}"
fi
...
if ! is_container_runtime_available; then
  ...
  if [ -n "${ANSIBLE_VAULT_PASSWORD_FILE_PATH:-""}" ]; then
    ANSIBLE_VAULT_PASSWORD_FILE_PATH="${ANSIBLE_DIRECTORY}/${ANSIBLE_VAULT_PASSWORD_FILE_NAME}"
    echo "Update ANSIBLE_VAULT_PASSWORD_FILE_PATH: ${ANSIBLE_VAULT_PASSWORD_FILE_PATH}"
  fi
fi

if [ "${ANSIBLE_VAULT_DECRYPT_MODE}" != "prompt" ]; then
  ANSIBLE_VAULT_DECRYPT_MODE="${ANSIBLE_VAULT_PASSWORD_FILE_PATH}"
  echo "Update ANSIBLE_VAULT_DECRYPT_MODE: ${ANSIBLE_VAULT_DECRYPT_MODE}"
fi
